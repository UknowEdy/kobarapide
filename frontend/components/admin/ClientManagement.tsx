...
import { ClientStatus, Role } from '../../types';
...
const ClientManagement: React.FC<ClientManagementProps> = ({ showToast }) => {
    ...
    const handleResendVerification = (userId: string, userEmail: string) => {
        resendVerificationEmail(userId);
        showToast(`Email de vérification renvoyé à ${userEmail}.`);
    };
        ...
        <Card title="Gestion des Clients">
            <div className="mb-4">
                ...
                >
                    ...
                    {/* FIX: Changed iteration from Object.entries to Object.values(ClientStatus) to resolve type inference error. */}
                    {Object.values(ClientStatus).map((status) => (
                        <option key={status} value={status}>{CLIENT_STATUS_CONFIG[status].text}</option>
                    ))}
                </select>
            </div>
