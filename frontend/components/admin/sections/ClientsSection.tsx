...
interface User {
  ...
  email: string;
  telephone: string;
  status: 'EN_ATTENTE' | 'ACTIF' | 'SUSPENDU' | 'BLOQUE' | 'REACTIVATION_EN_ATTENTE' | 'INACTIF_EXCLU' | 'EN_VERIFICATION_DOUBLON' | 'REJETE';
  score: number;
  role: string;
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex gap-4">
        {['ACTIF', 'SUSPENDU', 'EN_ATTENTE', 'BLOQUE'].map(status => (
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-700">
            {loading ? <tr><td colSpan={5} className="px-6 py-4 text-center">Chargement...</td></tr> : clients.length === 0 ? <tr><td colSpan={5} className="px-6 py-4 text-center text-gray-400">Aucun client</td></tr> : clients.map(c => (
              <tr key={c._id} className="hover:bg-gray-700"><td className="px-6 py-4">{c.nom} {c.prenom}</td><td className="px-6 py-4 text-sm text-gray-400">{c.email}</td><td className="px-6 py-4">{c.telephone}</td><td className="px-6 py-4"><span className={c.score >= 5 ? 'bg-green-900 text-green-200 px-2 py-1 rounded' : 'bg-blue-900 text-blue-200 px-2 py-1 rounded'}>{c.score}</span></td><td className="px-6 py-4"><select value={c.status} onChange={(e) => handleStatusChange(c._id, e.target.value)} className="bg-gray-700 text-white px-2 py-1 rounded"><option value="ACTIF">Actif</option><option value="SUSPENDU">Suspendu</option><option value="BLOQUE">Bloqué</option><option value="EN_ATTENTE">En Attente</option><option value="REJETE">Rejeté</option></select></td></tr>
            ))}
          </tbody>
        </table>
      </div>
